...
fn build_aliases(formula: &[Formula]) -> BTreeMap<String, String> {
  ...
  for f in formula {
    if let Some(old_name) = &f.old_name {
      aliases.insert(old_name.to_string(), f.name.to_string());
    }
    for alias_name in &f.aliases {
      aliases.insert(alias_name.to_string(), f.name.to_string());
    }
  }
  aliases
}
...
fn main() -> anyhow::Result<()> {
  ...
  let env = config::PacTree {
    ...
    packages: formula.into_iter().map(|i| (i.name.clone(), i)).collect(),
  };
  match sub {
